...
def _solve_nonlinear_strategic(cfg: SimpleNamespace, solver: ODILSolver,
    ...
    _sel = next((r for r in attempts_log if r.get("selected")), None)
    metrics["selected_elapsed_s"] = _sel["elapsed_s"] if _sel else None
    metrics["selected_iterations"] = _sel["iterations"] if _sel else None
    metrics["n_attempts"] = len(attempts_log)
    return metrics
...
def _solve_nonlinear_single(cfg, solver, bcs, material_params, initial_guess):
    
    reseed(cfg.seed)
    t0 = time.time()
    result = solver.solve(
        bcs=bcs, q=cfg.q, is_nonlinear=True, optim_name=cfg.optim_name,
        max_iter=cfg.max_iter_nonlinear, lr=cfg.lr, loss_weights=cfg.pde_weights,
        reg=cfg.reg_weight, verbose=False, bc_weight=cfg.bc_weight,
        print_every=cfg.print_every, gpu_monitor_interval=cfg.gpu_monitor_interval,
        initial_guess=initial_guess,
    )
    elapsed = time.time() - t0
    return _build_metrics(result, cfg, material_params, is_nonlinear=True, elapsed=elapsed)
...
def run_one_combination(scen: Dict[str, Any], opt: Dict[str, Any],
    ...
    lin_t = linear["_tensor_result"]
    linear.pop("_tensor_result", None)   
    linear_init = {
        "u_inner":   lin_t["u"][1:-1].detach().clone(),
        "w_inner":   lin_t["w"][1:-1].detach().clone(),
        "phi_inner": lin_t["phi"][1:-1].detach().clone(),
    }

    nl_by_strat: Dict[str, Dict[str, Any]] = {}
    for strat in P.INIT_STRATEGIES:
        ig = linear_init if strat == "linear" else None
        print(f"[nonlinear/{strat}] solving (single attempt)...")
        nl = _solve_nonlinear_single(cfg, solver, bcs, material_params, initial_guess=ig)
        nl_by_strat[strat] = nl
        print(f"[nonlinear/{strat}] w_max = {nl['w_max']:.6e}, "
              f"||R_PDE|| = {nl['R_PDE_norm']:.3e}, "
              f"iters = {nl['iterations']}, time = {nl['elapsed_s']:.2f} s")
        _write_displacement_csv(linear, nl, os.path.join(data_dir, f"displacement_{strat}.csv"))
        _write_loss_csv(linear["loss_history"], nl["loss_history"],
                        os.path.join(data_dir, f"loss_history_{strat}.csv"))

    
    nonlinear = nl_by_strat.get("linear") or next(iter(nl_by_strat.values()))
    ...
    summary = {
        ...
        "init_strategies": list(P.INIT_STRATEGIES),
        "linear":     {k: v for k, v in linear.items()
                       if k not in ("x", "u", "w", "phi", "loss_history")},
        "nonlinear":  {k: v for k, v in nonlinear.items()
                       if k not in ("x", "u", "w", "phi", "loss_history")},
        "nonlinear_by_strategy": {
            s: {k: v for k, v in nl.items() if k not in ("x", "u", "w", "phi", "loss_history")}
            for s, nl in nl_by_strat.items()
        },
        "displacement_csv": disp_csv,
        "loss_history_csv": loss_csv,
        "displacement_csv_by_strategy": {s: os.path.join(data_dir, f"displacement_{s}.csv") for s in nl_by_strat},
        "loss_history_csv_by_strategy": {s: os.path.join(data_dir, f"loss_history_{s}.csv") for s in nl_by_strat},
        "nonlinear_attempts": [],
        "nonlinear_attempts_csv": None,
    }
...
def main() -> int:
    ...
    results_root = os.path.abspath(os.path.join(PKG_ROOT, "results", "odil"))
