...
def build_strong_form_plate_system(config, material):
    if config.get("NONDIM"):
        return _build_strong_form_nondim(config, material)
    Nx = config["Nx"]
    Ny = config["Ny"]
    Ng = Nx * Ny
    a = config["a"]
...
def _build_strong_form_nondim(config, material):
    ...
    Z = np.zeros((Ng, Ng)); I = np.eye(Ng)

    Bmb = np.block([
        [(1.0 / lam1) * Dx, Z, Z, Z, Z],
        [Z, (lam2 / lam1) * Dy, Z, Z, Z],
        [(lam2 / lam1) * Dy, (1.0 / lam1) * Dx, Z, Z, Z],
        [Z, Z, Z, (1.0 / lam1) * Dx, Z],
        [Z, Z, Z, Z, (lam2 / lam1) * Dy],
        [Z, Z, Z, (lam2 / lam1) * Dy, (1.0 / lam1) * Dx],
    ])
    Bs = np.block([
        [Z, Z, (1.0 / lam1) * Dx, I, Z],
        [Z, Z, (lam2 / lam1) * Dy, Z, I],
    ])
    ...
    Rop = np.kron(C6, I) @ Bmb
    Qd = np.kron(As2, I) @ Bs
    Nx_op = Rop[0 * Ng:1 * Ng]; Ny_op = Rop[1 * Ng:2 * Ng]; Nxy_op = Rop[2 * Ng:3 * Ng]
    Mx_op = Rop[3 * Ng:4 * Ng]; My_op = Rop[4 * Ng:5 * Ng]; Mxy_op = Rop[5 * Ng:6 * Ng]
    Qx_op = Qd[0 * Ng:1 * Ng]; Qy_op = Qd[1 * Ng:2 * Ng]

    EqU = Dx @ Nx_op + lam2 * (Dy @ Nxy_op)
    EqV = Dx @ Nxy_op + lam2 * (Dy @ Ny_op)
    EqW = lam1 * (Dx @ Qx_op) + lam1 * lam2 * (Dy @ Qy_op)
    k_w = config.get("k_w", 0.0); k_s = config.get("k_s", 0.0)
    cw = k_w * a ** 2 / A110
    cs = k_s / A110
    if abs(k_w) > 1.0e-14 or abs(k_s) > 1.0e-14:
        EqW[:, 2 * Ng:3 * Ng] = EqW[:, 2 * Ng:3 * Ng] - cw * I + cs * (Dxx + lam2 ** 2 * Dyy)
    EqFx = Dx @ Mx_op + lam2 * (Dy @ Mxy_op) - lam1 * Qx_op
    EqFy = Dx @ Mxy_op + lam2 * (Dy @ My_op) - lam1 * Qy_op
    KL_full = -np.vstack([EqU, EqV, EqW, EqFx, EqFy])

    f_full = np.zeros(5 * Ng)
    f_full[2 * Ng:3 * Ng] = config.get("transverse_load", 0.0) * a ** 2 / (h * A110)
    ...
    KL_red = T.T @ KL_full @ T
    f_red = T.T @ f_full
    M_full = np.zeros((total, total)); KG_full = np.zeros((total, total))
    ...
    return {
        ...
        "x_nodes": x_nodes, "y_nodes": y_nodes,
        "KL_full": KL_full, "M_full": M_full, "KG_full": KG_full, "f_full": f_full,
        "KL_red": KL_red, "M_red": T.T @ M_full @ T, "KG_red": T.T @ KG_full @ T, "f_red": f_red,
        "transform": T, "free_dofs": free_dofs,
        "essential_dofs": np.array(sorted(ess_set)), "ops": ops,
