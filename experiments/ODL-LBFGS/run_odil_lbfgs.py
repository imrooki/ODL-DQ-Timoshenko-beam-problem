"""
ODIL-LBFGS sweep runner (adaptive perturbation restart DISABLED).

Drives the ODIL-LBFGS sweep with the in-loop adaptive perturbation restart
turned off, so that the LBFGS nonlinear solve can be observed WITHOUT that
restart. Parameters are read from params_odil_lbfgs.py (LBFGS only,
perturbation_enabled = False), and all outputs are written under this folder.

Runs ODIL-DQ on the two foundation scenarios (with / without elastic
foundation). For each scenario it solves the linear problem (warmup) and
then the nonlinear problem via the project's multi-attempt strategy
(num_solution_attempts = 3), records max|w|, final PDE residual norm,
iteration count and wall-clock time, and writes outputs to

    experiments/ODL-LBFGS/results/odil/<scenario>/lbfgs/

Outputs per run:
    - displacement CSV (linear + nonlinear, x, u, w, phi)
    - loss history CSV
    - JSON summary with all metrics (incl. per-attempt detail)

Usage:
    conda activate claude_test
    cd experiments/ODL-LBFGS
    python run_odil_lbfgs.py
...
# ---------------------------------------------------------------------------
# Path bootstrap: this script lives at <bundle>/experiments/ODL-LBFGS/
# Add the Opensource_2 bundle root so `modules.*` and `utils.*` resolve.
...
import params_odil_lbfgs as P  # noqa: E402
